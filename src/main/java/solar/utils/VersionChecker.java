...
public class VersionChecker
{
    ...
    public static Result doVersionCheck()
    {
        try
        {
            ModMetadata metadata = FabricLoader.getInstance().getModContainer(SolarApocalypseMod.MODID).get().getMetadata();
            ...
            if (result == Result.BEHIND)
            {
                ...
                if (changelogSection != null)
                {
                    ...
                    changelogSection.forEachEntry((key, value) ->
                    {
                        ...
                        {
                            SolarApocalypseMod.LOGGER.error("Could not parse changelog entry version key!");
                            e.printStackTrace();
                        }
                    });
        ...
        catch (IOException | VersionParsingException e)
        {
            ...
            {
                SolarApocalypseMod.LOGGER.error("Version check timed out!");
            }
            else
            {
                SolarApocalypseMod.LOGGER.error("Failed to perform version check!");
                e.printStackTrace();
            }
